module PdfHelper
  ...
  def make_pdf(options = {})
    ...
    html_string = html_string.gsub(".com:/",".com/") # strip out bad attachment_fu URLs
    html_string = html_string.gsub( /src=["']+([^:]+?)["']/i ) { |m| "src=\"#{Rails.public_path}/" + $1 + '"' } # re-route absolute paths
    
    # Remove asset ids on images with a regex
    html_string = html_string.gsub( /src=["'](\S+\?\d*)["']/i ) { |m| 'src="' + $1.split('?').first + '"' }
